...
type ElasticAgentRepository interface {
	...
	GetServerStats(ctx context.Context, serverID int, startTime time.Time, endTime time.Time) (*ServerStats, error)
}
...
func (r *elasticAgentRepository) GetServerStats(ctx context.Context, serverID int, startTime time.Time, endTime time.Time) (*ServerStats, error) {
	query := fmt.Sprintf(`
		{
			"size": 0,
			"query": {
